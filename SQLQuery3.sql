...
CREATE TABLE [NGƯỜI DÙNG] (
	...
	[Họ và tên] nvarchar(50) NOT NULL,
	[Ngày sinh] date,
	Sđt char(10) check(Sđt like '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'), 
...
CREATE TABLE [KHOÁ HỌC] (
	...
	[Tên khoá học] nvarchar(50) not null unique, 
...
CREATE TABLE [HỌC VẤN] (
	...
	[Bậc học] nvarchar(25) check([Bậc học] in ('Tiểu học','THCS','THPT','Đại học')), 
...
CREATE TABLE [CHỨNG CHỈ] (
	...
	[Tên khoá học] nvarchar(50) references [KHOÁ HỌC]([TÊN KHOÁ HỌC]) not null, 
...
CREATE TABLE [BẰNG CẤP] (
    ...
    [Học vị] nvarchar(20) not null check([Học vị] in ('Cử nhân','Thạc sĩ','Tiến sĩ')),
	[Chuyên ngành] nvarchar(50) not null, 
	[Tên trường] nvarchar(100) not null,
	PRIMARY KEY([Mã giảng viên], [Học vị], [Chuyên ngành], [Tên trường])
);

CREATE TABLE [KINH NGHIỆM GIẢNG DẠY] (
    [Mã giảng viên] int references [GIẢNG VIÊN]([MÃ GIẢNG VIÊN]),
    [Nơi công tác] nvarchar(100) not null, 
...
CREATE TABLE [BÀI HỌC] (
	...
	[Số thứ tự] tinyint IDENTITY(1,1),
	[Tên bài học] nvarchar(50) not null,
	[Bài tập] varchar(30),
...
CREATE TABLE [GIÁO TRÌNH] (
	...
	[Nhà xuất bản] nvarchar(50),
	[Tên giáo trình] nvarchar(50) not null,
